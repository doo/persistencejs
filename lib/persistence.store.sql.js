...
function config(persistence, dialect) {
  ...
  persistence.DbQueryCollection.prototype.list = function (tx, callback) {
    ...
    var sql = "SELECT " + selectFields.join(", ") + this.fromSql(mainAlias) + " " + joinSql + " " + whereSql;
  ...
  persistence.DbQueryCollection.prototype.destroyAll = function (tx, callback) {
    ...
    var selectSql = "SELECT id" + this.fromSql() + " " + joinSql + ' ' + whereSql;
    var deleteSql = "DELETE" + this.fromSql() + " " + joinSql + ' ' + whereSql;
  ...
  persistence.DbQueryCollection.prototype.count = function (tx, callback) {
    ...
    if (skipLimitSql === '') {
      var sql = "SELECT COUNT(*) AS cnt" + this.fromSql("root") + " " + joinSql + " " + whereSql;
    } else {
      var sql = "SELECT COUNT(*) AS cnt FROM (SELECT id " + this.fromSql("root") + " " + joinSql + " " + whereSql + skipLimitSql + ")";
    }
