...
function config(persistence, dialect) {
  ...
  persistence.DbQueryCollection.prototype.clone = function (cloneSubscribers) {
  ...
  persistence.DbQueryCollection.prototype.init = function(session, entityName, constructor) {
  ...
  persistence.DbQueryCollection.prototype.toUniqueString = function() {
  ...
  persistence.DbQueryCollection.prototype.joinSql = function (mainAlias) {
    return this._additionalJoinSqls.join(' ');
  }

  persistence.DbQueryCollection.prototype.whereSql = function (mainAlias, args) {
    var meta = persistence.getMeta(this._entityName);
    return " WHERE " + [ this._filter.sql(meta, mainAlias, args) ].concat(this._additionalWhereSqls).join(' AND ');
  }

  persistence.DbQueryCollection.prototype.groupSql = function (mainAlias, args) {
    return this._additionalGroupSqls.join(' ');
  }
