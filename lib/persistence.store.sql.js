...
function config(persistence, dialect) {
  ...
  persistence.RelationshipFilter.prototype.sql = function (meta, alias, values) {
    var rel = meta.hasMany[this.relationship];
    ...
    var mtm = {
        table: rel.tableName,
        prop: direct + '_' + this.relationship,
        inverseProp: inverse + '_' + rel.inverseProperty
      };

    var aliasPrefix = alias ? "`" + alias + "`." : "";

    var selectJoinTable = 'SELECT ' + mtm.prop + ' FROM ' + mtm.table + ' AS `jtable` WHERE `jtable`.' + mtm.inverseProp + ' IN ';
    var selectRelation = 'SELECT `rel`.id' + this.coll.fromSql("rel") + this.coll.joinSql("rel") + this.coll.whereSql("rel", values);

    return aliasPrefix + 'id IN (' + selectJoinTable + '(' + selectRelation + '))';
  };
  ...
  persistence.DbQueryCollection.prototype.destroyAll = function (tx, callback) {
    ...
    var selectSql = "SELECT id" + this.fromSql() + " " + joinSql + this.whereSql(null, args);
    var deleteSql = "DELETE" + this.fromSql() + " " + joinSql + this.whereSql(null, args);
  ...
  persistence.ManyToManyDbQueryCollection.prototype.clone = function (cloneSubscribers) {
    var c = persistence.QueryCollection.prototype.clone.call(this, cloneSubscribers);
